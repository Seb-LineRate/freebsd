...
kmem_1gig_find_free(struct kmem_1gig_page *p, vm_size_t size)
{
    ...
    while ((n->size > PAGE_SIZE) && (n->size/2 >= size)) {
        n->left_child = (void*)kmem_malloc_real(kmem_map, sizeof(struct kmem_1gig_free_node), M_NOWAIT);
        ...
        n->right_child = (void*)kmem_malloc_real(kmem_map, sizeof(struct kmem_1gig_free_node), M_NOWAIT);
...
kmem_1gig_add_page(void)
{
    ...
    va = kmem_malloc_1gig_page(kmem_map, M_NOWAIT);
    if (va == 0) {
    ...
    n = (void*)kmem_malloc_real(kmem_map, sizeof(struct kmem_1gig_free_node), M_NOWAIT);
    if (n == NULL) {
        kmem_free_real(kmem_map, va, one_gig);
        return NULL;
    }

    p = (void*)kmem_malloc_real(kmem_map, sizeof(struct kmem_1gig_page), M_NOWAIT);
    if (p == NULL) {
        kmem_free_real(kmem_map, (vm_offset_t)n, sizeof(struct kmem_1gig_free_node));
        kmem_free_real(kmem_map, va, one_gig);
        return NULL;
...
kmem_free_1gig_to_page(struct kmem_1gig_page *p, vm_offset_t addr, vm_size_t size)
{
    ...
    if (p->root == NULL) {
        p->root = (void*)kmem_malloc_real(kmem_map, sizeof(struct kmem_1gig_free_node), M_NOWAIT);
        if (p->root == NULL) {
